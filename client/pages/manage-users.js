...
export default function ManageUsers() {
  ...
  const toggleUserRole = async (userId, currentRole) => {
    ...
    try {
      console.log(`Updating user ${userId} role to ${newRole} via API`);
      await apiService.users.update(userId, { role: newRole });
  ...
  const toggleUserDisabled = async (userId, currentDisabled) => {
    ...
    try {
      console.log(`Updating user ${userId} disabled status to ${newDisabled} via API`);
      await apiService.users.update(userId, { disabled: newDisabled });
  ...
  const handleChangePassword = async (e) => {
    ...
    try {
      ...
      await apiService.users.updatePassword(user.uid, newPassword);
